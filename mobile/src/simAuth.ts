/**
 * ============================================================
 *  ماژول احراز هویت با سیم‌کارت — React Native
 * ------------------------------------------------------------
 *  ⚠️ واقعیت فنی مهم:
 *  خواندن «شماره» سیم‌کارت روی اندروید تضمینی نیست، چون اپراتورها
 *  (به‌ویژه همراه اول و ایرانسل) فیلد MSISDN را روی سیم‌کارت نمی‌نویسند.
 *  در iOS هم اپل هیچ API برای این کار ندارد.
 *
 *  بنابراین این ماژول سه لایه دارد:
 *   ۱) تلاش برای خواندن مستقیم شماره (اگر اپراتور نوشته باشد)
 *   ۲) خواندن «اثر انگشت سیم‌کارت» (ICCID/IMSI/اپراتور) که پایدار است
 *      و تغییر سیم‌کارت را تشخیص می‌دهد
 *   ۳) در صورت نبود شماره → ورود با کد پیامکی که خودکار خوانده می‌شود
 *      (روشی که واتساپ و تلگرام هم استفاده می‌کنند)
 * ============================================================
 */
...
/** باید دقیقاً با MOBILE_APP_SECRET روی سرور یکسان باشد */
...
export async function requestSimPermissions(): Promise<boolean> {
  ...
  try {
    const perms = [
      ...
      // در اندروید ۱۱+ برای خواندن شماره لازم است
...
export type SimInfo = {
  /** شماره — ممکن است خالی باشد (اکثر اپراتورهای ایرانی) */
  phoneNumber: string;
  /** شناسه پایدار سیم‌کارت برای تشخیص تعویض سیم */
  fingerprint: string;
  carrier: string;
  simCount: number;
  hasSim: boolean;
...
/** خواندن اطلاعات سیم‌کارت از ماژول‌های در دسترس */
export async function readSimInfo(): Promise<SimInfo> {
  ...
  if (Platform.OS !== "android") {
    // iOS: خواندن شماره ممکن نیست؛ فقط نام اپراتور در دسترس است
  ...
  // ❶ react-native-sim-data (اگر نصب باشد) — کامل‌ترین اطلاعات
  try {
    ...
    /* ماژول نصب نیست یا مجوز ندارد */
  }

  // ❷ react-native-device-info به‌عنوان پشتیبان
...
/* ------------------------------------------------------------------ */
/*  شناسه پایدار دستگاه (در Keychain امن ذخیره می‌شود)                  */
/* ------------------------------------------------------------------ */
...
export async function getDeviceId(): Promise<string> {
  ...
  const fresh = (await DeviceInfo.getUniqueId().catch(() => "")) || `${Date.now().toString(36)}${Math.random().toString(36).slice(2)}`;
...
export async function loginWithSim(): Promise<LoginResult> {
  ...
  if (!sim.hasSim) {
    return { status: "error", message: "سیم‌کارتی در این گوشی شناسایی نشد. لطفاً سیم‌کارت را بررسی کنید.", code: "NO_SIM" };
  }

  // ❶ دریافت nonce یکبارمصرف (ضد Replay Attack)
  const nonceRes = await fetchJson("/api/mobile/nonce", { deviceId }).catch(() => null);
  if (!nonceRes?.ok) {
    return { status: "error", message: "ارتباط با سرور برقرار نیست. اتصال اینترنت را بررسی کنید." };
  }
  const nonce = String(nonceRes.data.nonce);
  // از ساعت سرور استفاده می‌کنیم تا اختلاف ساعت گوشی مشکل‌ساز نشود
  ...
  if (!res) return { status: "error", message: "ارتباط با سرور برقرار نیست." };
  ...
  return { status: "error", message: String(res.data.error ?? "ورود ناموفق بود"), code: res.data.code };
}

/* ------------------------------------------------------------------ */
/*  ورود با کد پیامکی (وقتی شماره سیم‌کارت خوانده نشود)                 */
/* ------------------------------------------------------------------ */
...
export async function verifyOtp(username: string, password: string, code: string) {
  ...
  return { ok: false, message: String(res.data.error ?? "کد نامعتبر است") };
}
