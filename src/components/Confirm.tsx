...
/** هوک تاییدیه: `const confirm = useConfirm(); if (!(await confirm({title:'...'}))) return;` */
        ...
        >
          ...
          >
            <div className="mb-2 flex items-start gap-3">
              <span className={`text-2xl ${state.danger ? "" : ""}`}>{state.danger ? "⚠️" : "❓"}</span>
            ...
            <div className="mt-4 flex gap-2">
              ...
              >
                {state.confirmText ?? "تایید"}
              </button>
              <button
                onClick={() => close(false)}
                className="flex-1 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-slate-700 ring-1 ring-slate-300 hover:bg-slate-50"
              >
