...
const KINDS = ["روزانه", "ساعتی", "استعلاجی", "بدون حقوق", "اضطراری"];
...
export default function LeaveScreen() {
  ...
  const submit = async () => {
    const res = await fetch("/api/leaves", {
      ...
      body: JSON.stringify({ kind, fromDate, toDate, days: Number(days) || 1, reason }),
    });
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.id} className="border-b border-slate-100">
                  ...
                  <td className="px-2 py-2">{toPersianDigits(r.days)}</td>
