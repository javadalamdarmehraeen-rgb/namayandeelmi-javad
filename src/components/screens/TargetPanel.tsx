...
/** پنل پیشرفت تارگت — در صفحه اصلی نماینده و گزارش‌ها نمایش داده می‌شود */
export default function TargetPanel({ userId, compact = false }: { userId?: number; compact?: boolean }) {
  ...
  if (active.length === 0 && !compact) {
    return (
      <Card>
        <SectionTitle icon="🎯">تارگت فروش</SectionTitle>
        <p className="py-4 text-center text-xs text-slate-400">
          برای این دوره تارگتی تعریف نشده است. مدیر می‌تواند از صفحه «تارگت‌ها» تعریف کند.
        </p>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <SectionTitle icon="🎯">پیشرفت تارگت فروش</SectionTitle>
        <div className="flex items-center gap-2">
          ...
          <Badge tone={percent >= 100 ? "green" : percent >= 60 ? "amber" : "slate"}>
            تحقق کل: {toPersianDigits(percent)}٪
          </Badge>
        </div>
      </div>
      ...
      {totals ? (
        <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {[
            ["تارگت", toPersianDigits(totals.quantity), "bg-slate-50 text-slate-700"],
            ["فروش", toPersianDigits(totals.sold), "bg-teal-50 text-teal-700"],
            ["باقیمانده", toPersianDigits(totals.remaining), "bg-amber-50 text-amber-700"],
            ["جایزه", toPersianDigits(totals.bonus), "bg-emerald-50 text-emerald-700"],
          ].map(([l, v, c]) => (
          ...
          <div key={r.productKey} className="rounded-xl bg-slate-50 p-2 ring-1 ring-slate-200">
            <div className="flex flex-wrap items-center gap-2 text-[11px]">
              ...
              <span className="text-slate-500">
                تارگت <b className="text-slate-700">{toPersianDigits(r.quantity)}</b>
              </span>
              <span className="text-teal-700">
                فروش <b>{toPersianDigits(r.sold)}</b>
              </span>
              <span className={r.remaining === 0 ? "text-emerald-700" : "text-rose-700"}>
                باقیمانده <b>{toPersianDigits(r.remaining)}</b>
              </span>
            ...
            <div className="mt-1 h-1.5 overflow-hidden rounded bg-slate-200">
              <div
                className={`h-full transition-all ${r.percent >= 100 ? "bg-emerald-500" : r.percent >= 60 ? "bg-teal-500" : "bg-amber-500"}`}
      ...
      {totals && (totals.soldValueDistributor > 0 || totals.valueDistributor > 0) ? (
        <div className="mt-3 grid grid-cols-1 gap-2 text-[11px] sm:grid-cols-2">
          <div className="rounded-xl bg-slate-50 p-2.5 ring-1 ring-slate-200">
            ارزش تارگت (پخش): <b className="text-slate-800">{money(totals.valueDistributor)}</b> ریال
            <div className="text-teal-700">
              فروش: <b>{money(totals.soldValueDistributor)}</b> ریال
            </div>
          </div>
          <div className="rounded-xl bg-slate-50 p-2.5 ring-1 ring-slate-200">
            ارزش تارگت (داروخانه): <b className="text-slate-800">{money(totals.valuePharmacy)}</b> ریال
            <div className="text-teal-700">
              فروش: <b>{money(totals.soldValuePharmacy)}</b> ریال
            </div>
          </div>
        </div>
      ) : null}
