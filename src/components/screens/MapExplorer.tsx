...
const KIND_META: Record<Place["kind"], { label: string; icon: string; color: string }> = {
  pharmacy: { label: "داروخانه", icon: "🏥", color: "#0f766e" },
  doctor: { label: "پزشک", icon: "🩺", color: "#0369a1" },
  home: { label: "منزل نماینده", icon: "🏠", color: "#7c3aed" },
};

/**
 * نقشه جامع: نمایش داروخانه‌ها، پزشکان و منازل روی نقشه ایران
 * با فیلتر استان / شهر / منطقه / نماینده و جستجوی نام.
 */
export default function MapExplorer({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    for (const r of hm) push("home", r, String(r.title ?? "منزل"), String(r.repName ?? ""));
  ...
  /** محدوده‌ای که نقشه روی آن متمرکز می‌شود */
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionTitle icon="🗺️">نقشه جامع — داروخانه‌ها، پزشکان و منازل</SectionTitle>
      ...
      <Card>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Field label="شهر">
            <Combobox
              ...
              options={cities}
              selectOnly
              placeholder={province ? "همه شهرهای این استان" : "ابتدا استان را انتخاب کنید"}
            />
          </Field>
          <Field label="منطقه">
            <Combobox
          ...
          <Field label={isAdmin ? "نماینده" : "جستجو"}>
            {isAdmin ? (
              <select
                value={rep}
                onChange={(e) => {
              ...
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="🔍 نام، آدرس..." />
            )}
          </Field>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {isAdmin ? (
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="🔍 جستجو..." className="max-w-[200px]" />
          ) : null}
          <div className="flex-1" />
          <Badge tone="slate">محدوده: {area.name ?? IRAN_CENTER.name}</Badge>
      ...
      {loading ? <Alert kind="info">در حال بارگذاری نقاط...</Alert> : null}
      {!loading && filtered.length === 0 ? (
        <Alert kind="info">
          نقطه‌ای با این فیلترها یافت نشد. توجه کنید فقط رکوردهایی که لوکیشن ثبت‌شده دارند روی نقشه می‌آیند.
        </Alert>
      ) : null}

      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <MapBox height={520} points={mapPoints} area={area} follow={!!selected} />
          {selected ? (
        ...
        <Card className="lg:col-span-1">
          <h3 className="mb-2 text-sm font-bold text-slate-700">
            فهرست نقاط ({toPersianDigits(filtered.length)})
          </h3>
          <div className="max-h-[520px] space-y-1 overflow-y-auto">
            ...
            {filtered.length > 300 ? (
              <p className="py-2 text-center text-[11px] text-slate-400">
                {toPersianDigits(filtered.length - 300)} مورد دیگر — فیلتر را دقیق‌تر کنید
              </p>
            ) : null}
          </div>
        </Card>
