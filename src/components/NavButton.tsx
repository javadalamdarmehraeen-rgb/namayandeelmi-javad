  ...
  const menu = open ? (
    ...
    >
      <div
        className={
          mobileMenu
            ? "fade-in fixed inset-x-3 bottom-3 max-h-[78vh] overflow-y-auto rounded-3xl bg-white p-3 shadow-2xl ring-1 ring-white/80"
      ...
      >
        <div className="mb-2 flex items-center justify-between gap-2 px-1">
          <div>
            <div className="text-xs font-black text-slate-800">🧭 انتخاب مسیریاب</div>
            <div className="mt-0.5 max-w-[190px] truncate text-[10px] text-slate-400">{label}</div>
          </div>
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="flex size-8 items-center justify-center rounded-full bg-slate-100 text-sm font-bold text-slate-600"
            aria-label="بستن"
          >
            ✕
          </button>
        ...
        <div className={mobileMenu ? "grid grid-cols-2 gap-2" : "space-y-1"}>
          {list.map((a) => (
            <button
              ...
              type="button"
              onClick={() => go(a)}
              className={`flex items-center gap-2 rounded-xl bg-slate-50 px-3 text-right font-bold text-slate-700 ring-1 ring-slate-200 transition hover:bg-teal-50 hover:text-teal-800 ${
        ...
        <div className={mobileMenu ? "grid grid-cols-2 gap-2" : "space-y-1"}>
          {canShare ? (
            <button
              ...
              className="rounded-xl bg-sky-50 px-3 py-2.5 text-right text-[11px] font-bold text-sky-700 ring-1 ring-sky-200"
            >
              📤 اشتراک‌گذاری
            </button>
          ) : null}
          <button
            type="button"
            onClick={copyCoords}
            className="rounded-xl bg-slate-100 px-3 py-2.5 text-right text-[11px] font-bold text-slate-700 ring-1 ring-slate-200"
          >
            📋 کپی مختصات
          </button>
        </div>
    ...
    <>
      <span className="relative inline-block">
        <button
          ...
          className={`rounded-xl bg-gradient-to-l from-teal-600 to-emerald-600 font-bold text-white shadow-sm transition hover:from-teal-700 hover:to-emerald-700 active:scale-95 ${
