...
const fmtSize = (n: number) =>
  n > 1024 * 1024 ? `${toPersianDigits((n / 1024 / 1024).toFixed(1))} مگابایت` : `${toPersianDigits(Math.round(n / 1024))} کیلوبایت`;
...
export function printFile(id: number, mime: string) {
  ...
  const content = mime.startsWith("image/")
    ...
    : `<iframe src="${url}" style="border:0;width:100%;height:95vh" onload="window.focus();setTimeout(()=>window.print(),600)"></iframe>`;
  w.document.write(
    `<html dir="rtl"><head><meta charset="utf-8"><title>چاپ فایل</title></head><body style="margin:0">${content}</body></html>`,
  );
  w.document.close();
}
...
export function FileList({
  ...
  if (files.length === 0) return <p className="text-xs text-slate-400">فایلی بارگذاری نشده است</p>;
  return (
...
/** بارگذاری فایل/عکس. تا زمان ثبت رکورد، فایل‌ها بدون مالک ذخیره و سپس متصل می‌شوند. */
  ...
  const upload = async (list: FileList | null) => {
    ...
    for (const file of Array.from(list).slice(0, 5)) {
      if (file.size > 3 * 1024 * 1024) {
        setErr(`«${file.name}» بیش از ۳ مگابایت است`);
        continue;
      }
      ...
      if (res.ok) {
        ...
        setErr(d.error ?? "خطا در بارگذاری");
      }
    }
    setBusy(false);
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="soft" onClick={() => inputRef.current?.click()} disabled={busy}>
          {busy ? "⏳ در حال بارگذاری..." : "📎 انتخاب عکس یا فایل"}
        </Button>
        <Badge tone="slate">{toPersianDigits(files.length)} فایل</Badge>
        <span className="text-[11px] text-slate-400">عکس، PDF و آفیس — حداکثر ۳ مگابایت</span>
      </div>
