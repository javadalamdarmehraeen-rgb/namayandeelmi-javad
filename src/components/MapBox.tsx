"use client";

import { useEffect, useRef } from "react";
import "leaflet/dist/leaflet.css";
import type { Map as LeafletMap, Layer } from "leaflet";

export type MapPoint = { lat: number; lng: number; label?: string; color?: string };
export type MapArea = { lat: number; lng: number; zoom?: number; radiusKm?: number; name?: string };
export type ProvinceFeature = {
  type: "Feature";
  properties: { province_name?: string; name_fa?: string; [key: string]: unknown };
  geometry: { type: "Polygon" | "MultiPolygon"; coordinates: unknown };
};
export type ProvinceCollection = { type: "FeatureCollection"; features: ProvinceFeature[] };

/** نقشه سبک Leaflet + OSM با پشتیبانی از مرز واقعی استان‌های ایران */
export default function MapBox({
  points = [],
  path = [],
  onPick,
  height = 260,
  center,
  zoom = 14,
  follow = false,
  accuracy,
  draggable = false,
  area,
  provinces,
  selectedProvince = "",
  onProvinceClick,
  showProvinceLabels = true,
}: {
  points?: MapPoint[];
  path?: { lat: number; lng: number }[];
  onPick?: (p: { lat: number; lng: number }) => void;
  height?: number;
  center?: { lat: number; lng: number } | null;
  zoom?: number;
  follow?: boolean;
  accuracy?: number | null;
  draggable?: boolean;
  area?: MapArea | null;
  provinces?: ProvinceCollection | null;
  selectedProvince?: string;
  onProvinceClick?: (name: string) => void;
  showProvinceLabels?: boolean;
}) {
  const divRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<LeafletMap | null>(null);
  const layersRef = useRef<Layer[]>([]);
  const pickRef = useRef(onPick);
  const provinceClickRef = useRef(onProvinceClick);
  pickRef.current = onPick;
  provinceClickRef.current = onProvinceClick;

  const pointsKey = points.map((p) => `${p.lat.toFixed(6)},${p.lng.toFixed(6)},${p.label ?? ""}`).join(";");
  const pathKey = `${path.length}:${path.length ? `${path.at(-1)!.lat.toFixed(5)},${path.at(-1)!.lng.toFixed(5)}` : ""}`;
  const centerKey = center ? `${center.lat.toFixed(6)},${center.lng.toFixed(6)}` : "";
  const areaKey = area ? `${area.lat.toFixed(4)},${area.lng.toFixed(4)},${area.zoom ?? ""}` : "";
  const provinceKey = `${provinces?.features.length ?? 0}:${selectedProvince}:${showProvinceLabels}`;

  useEffect(() => {
    let disposed = false;
    (async () => {
      const L = (await import("leaflet")).default;
      if (disposed || !divRef.current || mapRef.current) return;
      const start = center ?? points[0] ?? path[0] ?? area ?? { lat: 32.4279, lng: 53.688 };
      const startZoom = points.length === 0 && path.length === 0 && area?.zoom ? area.zoom : zoom;
      const map = L.map(divRef.current, {
        attributionControl: false,
        zoomControl: true,
        preferCanvas: true,
        minZoom: 4,
        maxZoom: 19,
      }).setView([start.lat, start.lng], startZoom);
      L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", {
        maxZoom: 19,
        minZoom: 4,
        crossOrigin: true,
      }).addTo(map);
      map.on("click", (e: { latlng: { lat: number; lng: number } }) =>
        pickRef.current?.({ lat: e.latlng.lat, lng: e.latlng.lng }),
      );
      mapRef.current = map;
      setTimeout(() => map.invalidateSize(), 150);
      draw(L, map);
    })();
    return () => {
      disposed = true;
      mapRef.current?.remove();
      mapRef.current = null;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const L = (await import("leaflet")).default;
      const map = mapRef.current;
      if (!map || cancelled) return;
      draw(L, map);

      if (area && !follow) {
        map.setView([area.lat, area.lng], area.zoom ?? 10);
        return;
      }
      const target = center ?? points.at(-1) ?? path.at(-1);
      if (follow && target) map.setView([target.lat, target.lng], map.getZoom());
    })();
    return () => {
      cancelled = true;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pointsKey, pathKey, centerKey, areaKey, provinceKey, accuracy]);

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function draw(L: any, map: LeafletMap) {
    for (const layer of layersRef.current) layer.remove();
    layersRef.current = [];

    // مرزهای به‌هم‌پیوسته ۳۱ استان ایران
    if (provinces?.features?.length) {
      const provinceLayer = L.geoJSON(provinces, {
        style: (feature: ProvinceFeature) => {
          const name = feature.properties?.name_fa || String(feature.properties?.province_name || "").replace(/^استان\s+/, "");
          const selected = !!selectedProvince && name === selectedProvince;
          return {
            color: selected ? "#0f766e" : "#64748b",
            weight: selected ? 3 : 1.25,
            opacity: 0.95,
            fillColor: selected ? "#14b8a6" : "#99f6e4",
            fillOpacity: selected ? 0.32 : 0.1,
            dashArray: selected ? undefined : "2 2",
          };
        },
        onEachFeature: (feature: ProvinceFeature, layer: any) => {
          const name = feature.properties?.name_fa || String(feature.properties?.province_name || "").replace(/^استان\s+/, "");
          if (showProvinceLabels) {
            layer.bindTooltip(name, {
              permanent: true,
              direction: "center",
              className: `province-label${name === selectedProvince ? " province-label-active" : ""}`,
            });
          }
          layer.on("mouseover", () => {
            if (name !== selectedProvince) layer.setStyle({ fillOpacity: 0.25, weight: 2, color: "#0d9488" });
          });
          layer.on("mouseout", () => {
            if (name !== selectedProvince) layer.setStyle({ fillOpacity: 0.1, weight: 1.25, color: "#64748b" });
          });
          layer.on("click", (e: { originalEvent?: Event }) => {
            e.originalEvent?.stopPropagation?.();
            provinceClickRef.current?.(name);
            try {
              map.fitBounds(layer.getBounds(), { padding: [20, 20], maxZoom: 9 });
            } catch {
              /* ignore */
            }
          });
        },
      }).addTo(map);
      layersRef.current.push(provinceLayer);
    }

    if (path.length > 1) {
      const line = L.polyline(
        path.map((p) => [p.lat, p.lng]),
        { color: "#0d9488", weight: 4, opacity: 0.85 },
      ).addTo(map);
      layersRef.current.push(line);
    }

    if (accuracy && points[0]) {
      const circle = L.circle([points[0].lat, points[0].lng], {
        radius: accuracy,
        color: "#0ea5e9",
        weight: 1,
        fillColor: "#38bdf8",
        fillOpacity: 0.12,
      }).addTo(map);
      layersRef.current.push(circle);
    }

    for (const p of points) {
      const icon = L.divIcon({
        className: "",
        html: `<div style="background:${p.color ?? "#0f766e"};width:18px;height:18px;border-radius:50%;border:3px solid #fff;box-shadow:0 1px 7px rgba(15,23,42,.5)"></div>`,
        iconSize: [18, 18],
        iconAnchor: [9, 9],
      });
      const marker = L.marker([p.lat, p.lng], { icon, draggable }).addTo(map);
      if (p.label) marker.bindTooltip(p.label, { direction: "top", className: "place-label" });
      if (draggable) {
        marker.on("dragend", (e: { target: { getLatLng: () => { lat: number; lng: number } } }) => {
          const ll = e.target.getLatLng();
          pickRef.current?.({ lat: ll.lat, lng: ll.lng });
        });
      }
      layersRef.current.push(marker);
    }

    const all = [...points, ...path];
    if (!area && all.length > 1) {
      map.fitBounds(
        all.map((p) => [p.lat, p.lng] as [number, number]),
        { padding: [24, 24], maxZoom: 17 },
      );
    } else if (!area && all.length === 1) {
      map.setView([all[0].lat, all[0].lng], Math.max(map.getZoom(), 15));
    }
  }

  return (
    <div
      ref={divRef}
      style={{ height }}
      className="map-frame w-full overflow-hidden rounded-2xl ring-1 ring-slate-200"
      role="application"
      aria-label="نقشه تعاملی"
    />
  );
}
