...
export default function PanelHome() {
  ...
  const main = [
    { href: "/panel/pharmacies", title: "ثبت اطلاعات داروخانه", icon: "🏥", n: counts.ph, perm: "pharmacy", color: "from-teal-600 to-teal-500" },
    { href: "/panel/doctors", title: "ثبت اطلاعات پزشک", icon: "🩺", n: counts.dr, perm: "doctor", color: "from-sky-600 to-sky-500" },
    { href: "/panel/orders", title: "ثبت سفارشات داروخانه", icon: "🧾", n: counts.or, perm: "order", color: "from-indigo-600 to-indigo-500" },
  ].filter((c) => has(c.perm));

  const more = [
    { href: "/panel/trip", title: "تردد و ویزیت", icon: "🗺️", perm: "trip" },
    { href: "/panel/home", title: "لوکیشن منزل", icon: "🏡", perm: "home" },
    { href: "/panel/leaves", title: "درخواست مرخصی", icon: "📝", perm: "leave" },
    { href: "/panel/notifications", title: "اعلان‌ها", icon: "🔔", perm: "" },
    { href: "/panel/options", title: "افزودن مقادیر", icon: "➕", perm: "options" },
    { href: "/panel/reports", title: "گزارش ماهانه", icon: "📈", perm: "reports" },
  ].filter((c) => !c.perm || has(c.perm));

  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <h1 className="text-lg font-black text-slate-800">سلام {me.fullName} 👋</h1>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {main.map((c) => (
          <Link
            key={c.href}
            href={c.href}
            className={`flex items-center gap-4 rounded-2xl bg-gradient-to-l ${c.color} p-4 text-white shadow-md active:scale-[0.99]`}
          >
            ...
            <span className="min-w-0">
              ...
              <span className="mt-0.5 block text-[11px] text-white/85">{toPersianDigits(c.n)} رکورد</span>
            </span>
          </Link>
        ))}
      </div>
      ...
      {main.length + more.length === 0 ? (
        <div className="rounded-2xl bg-amber-50 p-4 text-sm text-amber-800 ring-1 ring-amber-200">
          هنوز دسترسی‌ای برای شما تعریف نشده است. با مدیر سیستم تماس بگیرید.
        </div>
      ) : null}
    </div>
  );
