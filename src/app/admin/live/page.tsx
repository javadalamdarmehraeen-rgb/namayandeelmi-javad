...
export default function LivePage() {
  ...
  const mapPoints = (focused ? [focused] : rows).map((r) => ({
    lat: r.lat,
    lng: r.lng,
    label: `${r.repName} — ${r.live ? "🟢 زنده" : "⚪ آخرین موقعیت"} (${ageLabel(r.ageMs)})`,
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionTitle icon="📍">موقعیت زنده نمایندگان</SectionTitle>
        <div className="flex flex-wrap gap-2">
          <Badge tone="green">🟢 آنلاین: {toPersianDigits(online)}</Badge>
          <Badge tone="amber">🚗 در حال ویزیت: {toPersianDigits(onTrip)}</Badge>
          <Button variant={autoRefresh ? "success" : "ghost"} onClick={() => setAutoRefresh((v) => !v)}>
            {autoRefresh ? "🔴 زنده (هر ۱۲ ثانیه)" : "⏸ متوقف"}
          </Button>
      ...
      <Alert kind="info">
        موقعیت هر نماینده حتی وقتی GPS خاموش یا اینترنت ضعیف باشد، از آخرین نقطه معتبر نگهداری می‌شود و به‌محض
        برقراری ارتباط بروز می‌گردد.
      </Alert>

      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-700">نمایندگان</h3>
            {focused ? (
          ...
          <div className="max-h-[460px] space-y-2 overflow-y-auto">
              ...
              >
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  <Badge tone={r.live ? "green" : r.stale ? "amber" : "slate"}>
                    {r.live ? "🟢 زنده" : r.stale ? "🟡 اخیر" : "⚪ قدیمی"}
                  </Badge>
                  {r.activeTrip ? <Badge tone="green">🚗 ویزیت</Badge> : null}
                  {!r.gpsOn ? <Badge tone="amber">GPS خاموش</Badge> : null}
                </div>
                <div className="mt-1 flex flex-wrap gap-2 text-[10px] text-slate-500">
                  <span>{ageLabel(r.ageMs)}</span>
                  {r.accuracy ? <span>دقت {toPersianDigits(Math.round(r.accuracy))}م</span> : null}
                  {r.speed ? <span>سرعت {toPersianDigits(Math.round(r.speed * 3.6))} km/h</span> : null}
                  {r.battery !== null ? <span>🔋 {toPersianDigits(r.battery)}٪</span> : null}
                  {r.activeTrip ? <span>مسافت {formatDistance(r.activeTrip.distanceM)}</span> : null}
                </div>
            ...
            {rows.length === 0 ? (
              <p className="py-6 text-center text-xs text-slate-400">هنوز موقعیتی دریافت نشده است</p>
            ) : null}

            {missing.length ? (
              <div className="rounded-xl bg-slate-100 p-2 text-[11px] text-slate-500">
                بدون موقعیت: {missing.map((m) => m.repName).join("، ")}
              </div>
            ) : null}
          </div>
        </Card>
        ...
        <Card className="lg:col-span-2">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <h3 className="text-sm font-bold text-slate-700">
              {focused ? `موقعیت ${focused.repName}` : "نقشه همه نمایندگان"}
            </h3>
            {focused ? (
          ...
          {focused ? (
            <div className="mt-3 grid grid-cols-2 gap-2 text-center text-[11px] sm:grid-cols-4">
              {[
                ["آخرین بروزرسانی", tehranTime(focused.updatedAt)],
                ["وضعیت GPS", focused.gpsOn ? "روشن" : "خاموش"],
                ["دقت موقعیت", focused.accuracy ? `${toPersianDigits(Math.round(focused.accuracy))} متر` : "—"],
                ["مسافت سفر", focused.activeTrip ? formatDistance(focused.activeTrip.distanceM) : "—"],
              ].map(([l, v]) => (
