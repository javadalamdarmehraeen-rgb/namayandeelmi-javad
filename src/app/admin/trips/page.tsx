...
export default function AdminTrips() {
  ...
  /* ---------- داروخانه‌ها، پزشکان و منازل برای نمایش روی نقشه ---------- */
  useEffect(() => {
    (async () => {
  ...
  /** مسافت محاسبه‌شده از روی نقاط (اگر سرور هنوز محاسبه نکرده باشد) */
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionTitle icon="🗺️">رصد تردد نمایندگان</SectionTitle>
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Input placeholder="🔍 جستجو..." value={q} onChange={(e) => setQ(e.target.value)} className="max-w-[200px]" />
          <Badge tone="slate">{toPersianDigits(filtered.length)} سفر</Badge>
          <span className="mr-auto text-[11px] text-slate-500">🏥 داروخانه · 🩺 پزشک · 🏠 منزل · ⏸ توقف</span>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <div className="max-h-[520px] space-y-3 overflow-y-auto">
              ...
              return (
                <div key={repName}>
                  <div className="mb-1 flex flex-wrap items-center gap-2 rounded-lg bg-slate-100 px-2 py-1.5">
                    ...
                    <Badge tone="slate">{toPersianDigits(list.length)} سفر</Badge>
                    {totalDist > 0 ? <Badge tone="green">{formatDistance(totalDist)}</Badge> : null}
                    {list.some((t) => t.status === "active") ? <Badge tone="green">🚗 فعال</Badge> : null}
                  </div>
                    ...
                    {list.map((t) => (
                      ...
                      >
                        ...
                        <div className="mt-1 flex flex-wrap gap-2 text-[10px] text-slate-500">
                          <span>شروع {tehranTime(t.startedAt)}</span>
                          {t.endedAt ? <span>پایان {tehranTime(t.endedAt)}</span> : null}
                          <span>{toPersianDigits(t.points ?? 0)} نقطه</span>
                          {t.distanceM ? <span>{formatDistance(t.distanceM)}</span> : null}
                          {t.stopSeconds ? <span>توقف {formatDuration(t.stopSeconds)}</span> : null}
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {grouped.length === 0 ? <p className="py-6 text-center text-sm text-slate-400">سفری یافت نشد</p> : null}
          </div>
        </Card>
          ...
          {selected ? (
            <>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                <Badge tone="amber">توقف {formatDuration(totalStop || selected.stopSeconds || 0)}</Badge>
                <Badge tone="slate">{toPersianDigits(points.length)} نقطه</Badge>
              </div>
                ...
                points={[
                  ...
                  ...points
                    ...
                    .map((p) => ({
                      ...
                      lng: p.lng,
                      label: `${
                        p.kind === "start" ? "▶️ شروع" : p.kind === "end" ? "🏁 پایان" : "⏸ توقف"
                ...
                {points
                  ...
                  .map((p) => (
                    <div key={p.id} className="flex flex-wrap items-center gap-2 rounded-lg bg-slate-50 px-3 py-1.5">
                      <span>{p.kind === "start" ? "▶️ شروع ویزیت" : p.kind === "end" ? "🏁 پایان سفر" : "⏸ توقف"}</span>
                      ...
                      {!p.gpsOn ? <Badge tone="slate">GPS خاموش</Badge> : null}
                      {p.note ? <span className="text-slate-600">{p.note}</span> : null}
                    </div>
                  ))}
                {points.length === 0 ? <p className="py-4 text-center text-slate-400">نقطه‌ای ثبت نشده است</p> : null}
              </div>
            </>
          ) : (
            <>
