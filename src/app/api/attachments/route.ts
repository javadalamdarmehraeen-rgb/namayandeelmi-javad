...
export async function POST(req: Request) {
  ...
  if (!match) return Response.json({ error: "فایل نامعتبر است" }, { status: 400 });
  const mimeType = match[1].slice(0, 100);
  const base64 = match[2];
  const size = Math.floor((base64.length * 3) / 4);
  if (size > MAX_BYTES) {
    return Response.json({ error: "حجم فایل نباید بیشتر از ۳ مگابایت باشد" }, { status: 400 });
  }
  const [row] = await db
    .insert(attachments)
    .values({
...
/** اتصال فایل‌های موقت (بدون مالک) به رکورد تازه ثبت‌شده */
export async function PATCH(req: Request) {
  ...
  if (!ids.length || !Number.isFinite(ownerId)) return Response.json({ error: "ورودی نامعتبر" }, { status: 400 });
  await db
    .update(attachments)
    .set({ ownerId })
