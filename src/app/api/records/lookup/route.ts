...
/** نرمال‌سازی نام برای مقایسه: حذف فاصله، نیم‌فاصله، «داروخانه»، «دکتر» و یکسان‌سازی حروف */
export function normalizeName(input: string) {
  return String(input ?? "")
    ...
    .replace(/[أإآا]/g, "ا")
    .replace(/ة/g, "ه")
    .replace(/^(داروخانه|دکتر|دكتر|درمانگاه|مطب)\s*/g, "")
    .replace(/\s+/g, "")
    .trim()
    .toLowerCase();
}
 ...
 *
 * دو کاربرد:
 *  ۱) تشخیص تکراری بودن هنگام ثبت داروخانه/پزشک جدید
 *  ۲) پرکردن خودکار فرم سفارش از روی سابقه همان داروخانه
 */
export async function GET(req: Request) {
  ...
  // اطلاعاتی که در فرم سفارش خودکار پر می‌شود (اولویت با آخرین سفارش)
  const suggestion =
    last || base
      ? {
