...
export async function POST(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    if (type === "orders") {
      ...
      // ارسال خودکار به پیام‌رسان‌های فعال (در پس‌زمینه — ثبت را معطل نمی‌کند)
        ...
        void (async () => {
          try {
            ...
            await notify({
              ...
              kind: "order",
              title: `🧾 سفارش جدید از ${row.repName}`,
...
/** آیا ارسال خودکار سفارش به پیام‌رسان‌ها فعال است؟ (پیش‌فرض: بله) */
...
/** ویرایش رکورد — نماینده فقط رکورد خودش، مدیر همه */
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  await log(user.id, user.fullName, "ویرایش رکورد", `${type} #${id}`);
...
export async function DELETE(req: Request, ctx: Ctx) {
  ...
  if (!isAdmin && !user.permissions.includes(perm)) {
    return Response.json({ error: "شما اجازه حذف ندارید. با مدیر تماس بگیرید." }, { status: 403 });
  }
  ...
  await log(user.id, user.fullName, "حذف رکورد", `${type} #${id}`);
  return Response.json({ ok: true });
}
