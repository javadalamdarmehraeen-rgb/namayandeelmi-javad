...
/**
 * جستجوی آدرس روی نقشه (Geocoding) — پروکسی سمت سرور.
 *
 * چرا از سرور؟ سرویس‌های نقشه معمولاً CORS اجازه نمی‌دهند و روی
 * اینترنت ایران هم گاهی مسدودند؛ با عبور از سرور، جستجو همیشه کار می‌کند.
 *
 * GET /api/geocode?q=داروخانه شفا تهران
 * GET /api/geocode?lat=35.7&lng=51.4        (جستجوی معکوس: مختصات → آدرس)
 */
...
async function photon(q: string): Promise<Hit[]> {
  // lat/lon مرکز ایران برای اولویت‌دهی نتایج داخل کشور
  const url =
  ...
  return (data.features ?? [])
    .filter((f) => {
      ...
      return !c || c === "IR"; // اولویت با نتایج ایران
    })
    .map((f) => {
...
export async function GET(req: Request) {
  ...
  // نکته: Number(null) برابر صفر است؛ پس باید وجود پارامتر جداگانه بررسی شود
  // وگرنه هر جستجوی متنی به‌اشتباه «جستجوی معکوس در مختصات ۰،۰» تلقی می‌شد.
  ...
  try {
    ...
    // اگر کاربر مستقیم مختصات وارد کند: «35.7219, 51.4089»
    const coord = q.match(/^\s*(-?\d{1,2}\.\d+)\s*[,،]\s*(-?\d{1,3}\.\d+)\s*$/);
    if (coord) {
      const la = Number(coord[1]);
      const ln = Number(coord[2]);
      return Response.json({
        results: [{ label: `مختصات دستی: ${la}, ${ln}`, lat: la, lng: ln, type: "coords" }],
      });
    }

    /**
     * چند سرویس پشت سر هم امتحان می‌شوند تا جستجو همیشه نتیجه بدهد.
     * Photon اول است چون محدودیت نرخ ندارد؛ Nominatim برای /search
     * سخت‌گیر است و گاهی درخواست‌های پیاپی را رد می‌کند.
     */
    ...
    return Response.json(
      { results: [], error: err instanceof Error ? err.message : "جستجو ناموفق بود" },
      { status: 200 },
    );
  }
}
