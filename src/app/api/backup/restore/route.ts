...
/** فقط ستون‌های موجود در جدول نگه داشته می‌شوند و تاریخ‌ها به Date تبدیل می‌شوند */
...
export async function POST(req: Request) {
  ...
  if (!data || typeof data !== "object" || !Array.isArray(data.users)) {
    return Response.json({ error: "فایل پشتیبان معتبر نیست" }, { status: 400 });
  }
  ...
  try {
    if (mode === "replace") {
      // ترتیب حذف مهم نیست چون کلید خارجی سخت‌گیرانه نداریم
      for (const t of [
        "attachments",
        "trip_points",
        "trips",
    ...
    // فایل‌ها فقط اگر محتوای base64 در پشتیبان باشد
    ...
    await db
      ...
      .values({
        ...
        action: "بازیابی پشتیبان",
        detail: `حالت: ${mode === "replace" ? "جایگزینی کامل" : "ادغام"} | ${JSON.stringify(report)}`.slice(0, 400),
      })
      .catch(() => undefined);
    ...
    return Response.json(
      { error: `خطا در بازیابی: ${err instanceof Error ? err.message : "نامشخص"}` },
      { status: 500 },
    );
  }
}
