...
export async function POST(req: Request) {
  ...
  if (!label) return Response.json({ error: "عنوان سمت الزامی است" }, { status: 400 });
  const key =
    String(b.key ?? "")
      .trim()
      .toLowerCase()
  ...
  if (dup.length) return Response.json({ error: "این شناسه سمت تکراری است" }, { status: 400 });
  const [row] = await db
    .insert(roles)
...
export async function DELETE(req: Request) {
  ...
  const r = rows[0];
  if (!r) return Response.json({ error: "سمت یافت نشد" }, { status: 404 });
  if (r.builtin) return Response.json({ error: "سمت‌های پایه قابل حذف نیستند" }, { status: 400 });
  const inUse = await db.select({ id: users.id }).from(users).where(eq(users.role, r.key)).limit(1);
  if (inUse.length) return Response.json({ error: "این سمت به کاربرانی اختصاص داده شده است" }, { status: 400 });
