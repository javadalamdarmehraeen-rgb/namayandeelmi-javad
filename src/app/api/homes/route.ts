...
export async function POST(req: Request) {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return Response.json({ error: "لوکیشن منزل را روی نقشه انتخاب کنید" }, { status: 400 });
  }
  const values = {
    userId: user.id,
    repName: user.fullName,
    title: String(b.title ?? "منزل").trim().slice(0, 160) || "منزل",
  ...
  await db
    .insert(activityLogs)
    .values({ userId: user.id, userName: user.fullName, action: "ثبت لوکیشن منزل", detail: values.title })
    .catch(() => undefined);
