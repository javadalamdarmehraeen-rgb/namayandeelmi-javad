...
/**
 * موقعیت زنده نمایندگان.
 *
 * POST → نماینده موقعیت خود را ارسال می‌کند (حتی وقتی سفر فعال نیست)
 * GET  → مدیر آخرین موقعیت همه نمایندگان را می‌بیند
 */
export async function POST(req: Request) {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return Response.json({ error: "مختصات نامعتبر" }, { status: 400 });
  }
...
export async function GET() {
  ...
  const enriched = scoped.map((r) => {
    ...
    return {
      ...r,
      ageMs,
      /** آنلاین: کمتر از ۳ دقیقه پیش گزارش داده */
  ...
  // نمایندگانی که هیچ موقعیتی نفرستاده‌اند
