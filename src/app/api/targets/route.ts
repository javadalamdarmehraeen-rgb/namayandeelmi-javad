...
export type TargetProgress = {
  ...
  /** تعداد فروخته‌شده در دوره */
  sold: number;
  /** تعداد جایزه داده‌شده */
  bonus: number;
  remaining: number;
  percent: number;
 ...
 * پیشرفت تارگت هر کالا: تعداد هدف، فروش انجام‌شده، باقیمانده و ارزش ریالی
 */
export async function GET(req: Request) {
  ...
  // تارگت‌های تعریف‌شده (دوره جاری + تارگت دائمی)
...
/**
 * PUT /api/targets — ثبت/بروزرسانی دسته‌ای تارگت‌های یک نماینده
...
export async function PUT(req: Request) {
  ...
  if (!Number.isFinite(userId)) return Response.json({ error: "نماینده نامعتبر" }, { status: 400 });

  const target = (await dbRetry(() => db.select().from(users).where(eq(users.id, userId)).limit(1), "targets:user"))[0];
  if (!target) return Response.json({ error: "نماینده یافت نشد" }, { status: 404 });
...
export async function DELETE(req: Request) {
  ...
  if (!Number.isFinite(userId)) return Response.json({ error: "شناسه نامعتبر" }, { status: 400 });
