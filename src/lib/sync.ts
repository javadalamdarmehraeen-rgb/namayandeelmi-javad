...
/* ============================================================
 *  موتور همگام‌سازی دوطرفه بین سرورها
 *
 *  جریان کار برای هر همتا:
 *    ۱) PULL  → تغییرات همتا از زمان pullCursor گرفته و اعمال می‌شود
 *    ۲) PUSH  → تغییرات محلی از زمان pushCursor برای همتا ارسال می‌شود
 *
 *  حل تعارض: Last-Write-Wins بر اساس updated_at
 *  ضد حلقه: رکوردی که origin آن خودِ ما باشد دوباره برنمی‌گردد
 * ============================================================ */
...
/**
 * تغییرات محلی پس از یک زمان مشخص.
 *
 * توجه: بر اساس origin فیلتر نمی‌کنیم؛ اگر رکوردی از همتا آمده و اینجا
 * ویرایش شده باشد، باید برگردد. جلوگیری از حلقه با دو مکانیزم انجام می‌شود:
 *   ۱) هنگام اعمال، زمان اصلی رکورد حفظ می‌شود (sek.sync = on)
 *   ۲) شرط `updated_at < excluded.updated_at` رکورد تکراری را رد می‌کند
 */
export async function collectChanges(since: Date | null, excludeOrigin?: string): Promise<ChangeSet> {
  ...
  for (const t of SYNC_TABLES) {
    ...
    try {
      ...
      console.warn(`[sync] خواندن ${t.name} ناموفق:`, err instanceof Error ? err.message : err);
    }
  }
  return out;
}
...
/* ------------------------------------------------------------------ */
/*  اعمال تغییرات دریافتی                                              */
/* ------------------------------------------------------------------ */

/** UPSERT بر اساس uid با قانون Last-Write-Wins */
export async function applyChanges(changes: ChangeSet, fromPeer: string) {
  let applied = 0;
  let skipped = 0;
  // یک اتصال اختصاصی تا set_config در طول کل عملیات معتبر بماند
  ...
  for (const [table, rows] of Object.entries(changes)) {
    ...
    for (const raw of rows.slice(0, MAX_BODY_ROWS)) {
      ...
      // فقط ستون‌هایی که در این سرور وجود دارند
      ...
      try {
        // فقط اگر نسخه دریافتی جدیدتر باشد جایگزین می‌شود.
        // set_config زمان اصلی رکورد را حفظ می‌کند تا تریگر آن را تازه نکند.
        const q = `
        ...
        skipped++;
        perTable[table].skipped++;
        console.warn(`[sync] اعمال ${table}/${uid} ناموفق:`, err instanceof Error ? err.message.slice(0, 120) : err);
      }
    }
  }

  // بازگرداندن پرچم به حالت عادی؛ در غیر این صورت اتصالِ بازگشته به استخر
  // باعث می‌شود ویرایش‌های بعدی کاربران زمان‌شان تازه نشود.
  try {
...
export async function syncWithPeer(peerRow: typeof syncState.$inferSelect): Promise<SyncReport> {
  ...
  try {
    if (!syncSecret()) throw new Error("SYNC_SECRET تنظیم نشده است");
    if (!peerRow.peerUrl) throw new Error("آدرس همتا تنظیم نشده است");

    /* ---------- ۱) PULL: گرفتن تغییرات همتا ---------- */
    const pullRes = await callPeer(peerRow.peerUrl, "/api/sync/pull", {
      ...
      excludeOrigin: me, // آنچه خودمان فرستاده بودیم برنگردد
    });
    ...
    notes.push(`دریافت ${pulled} رکورد`);

    /* ---------- ۲) PUSH: ارسال تغییرات محلی ---------- */
    const localChanges = await collectChanges(
      peerRow.pushCursor ? new Date(peerRow.pushCursor) : null,
      peerRow.peer, // آنچه از همتا گرفتیم دوباره پس نفرستیم
    );
    ...
    if (rowCount > 0) {
      ...
      // جدیدترین updated_at ارسال‌شده به‌عنوان مکان‌نما
      ...
      notes.push(`ارسال ${pushed} رکورد`);
    } else {
      newPushCursor = new Date();
      notes.push("تغییر محلی جدیدی نبود");
    }
    ...
    await dbRetrySafe(
      () =>
        db
          .update(syncState)
          .set({ lastSyncAt: new Date(), lastStatus: "✖ ناموفق", lastError: detail.slice(0, 400) })
          .where(eq(syncState.id, peerRow.id)),
      undefined,
      "sync:fail",
    );
...
/** همگام‌سازی با همه همتاهای فعال */
