...
/**
 * ============================================================
 *  ردیاب مسیر مقاوم (Offline-First GPS Tracker)
 * ------------------------------------------------------------
 *  اصول:
 *   • همه نقاط ابتدا در IndexedDB محلی ذخیره می‌شوند
 *   • ارسال به سرور در پس‌زمینه و با تلاش مجدد انجام می‌شود
 *   • قطعی اینترنت هیچ نقطه‌ای را از بین نمی‌برد و هشدار نمی‌دهد
 *   • خاموش شدن GPS، ردیابی را متوقف نمی‌کند؛ آخرین موقعیت معتبر
 *     نگه داشته می‌شود و با روشن شدن دوباره ادامه می‌یابد
 *   • توقف بیش از ۲ دقیقه در یک نقطه به‌عنوان «توقف» ثبت می‌شود
 * ============================================================
 */
...
const STORE = "points";
const MIN_DISTANCE_M = 12; // کمتر از این جابه‌جایی ثبت نمی‌شود (نویز GPS)
const STOP_THRESHOLD_MS = 2 * 60 * 1000; // توقف بیش از ۲ دقیقه
...
export async function bufferPoint(tripId: number, p: TrackPoint) {
  try {
    ...
    /* حافظه در دسترس نیست — نقطه از دست می‌رود ولی برنامه متوقف نمی‌شود */
  }
}
...
export function formatDistance(m: number) {
  return m >= 1000 ? `${(m / 1000).toFixed(1)} کیلومتر` : `${Math.round(m)} متر`;
}
...
export function formatDuration(sec: number) {
  ...
  return `${Math.round(sec)} ثانیه`;
}

/* ------------------------------ همگام‌سازی ------------------------------ */

/** ارسال نقاط ذخیره‌شده به سرور. در صورت قطعی، نقاط دست‌نخورده می‌مانند. */
export async function flushPoints(): Promise<{ sent: number; pending: number }> {
  ...
  for (const [tripId, points] of byTrip) {
    ...
    try {
      ...
      break; // آفلاین — بقیه بماند برای بعد
    }
  }
  return { sent, pending: (await bufferedPoints()).length };
}

/** ارسال موقعیت زنده (مستقل از سفر) — خطا نادیده گرفته می‌شود */
...
/**
 * ردیاب مسیر. با `start()` شروع و با `stop()` پایان می‌یابد.
 * قطع GPS یا اینترنت باعث توقف ردیابی نمی‌شود.
 */
export function createTracker({ tripId, onUpdate }: TrackerOptions) {
    ...
    if (dist < MIN_DISTANCE_M) {
      ...
      if (stopped >= STOP_THRESHOLD_MS && now - lastSavedAt >= STOP_THRESHOLD_MS) {
        ...
        await record({
          ...
          accuracy: pos.coords.accuracy,
          kind: "pause",
          note: `توقف ${formatDuration(stopped / 1000)}`,
  ...
  const onError = () => {
    // GPS خاموش یا بدون سیگنال — ردیابی ادامه دارد
    state.gpsOn = false;
    emit();
  };
  ...
  const start = () => {
    ...
    // ارسال دوره‌ای نقاط ذخیره‌شده (بی‌صدا)
    ...
    // ارسال موقعیت زنده حتی وقتی GPS خطا می‌دهد
  ...
  const stop = async (note = "پایان ویزیت") => {
    if (watchId !== null) {
