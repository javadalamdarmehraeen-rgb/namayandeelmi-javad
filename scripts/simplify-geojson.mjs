#!/usr/bin/env node
/** ساده‌سازی مرزهای استان‌ها با الگوریتم Ramer–Douglas–Peucker */
...
function simplify(points, eps) {
  ...
  // برای حلقه بسته، از دورترین نقطه نسبت به نقطه اول نصف می‌کنیم تا شکل حفظ شود
  let split = 0;
  if (wasClosed) {
    let max = -1;
...
for (const feature of geo.features) {
  ...
  const name = raw.replace(/^استان\s+/, "");
  feature.properties.name_fa = name === "کهگیلویه و بویر احمد" ? "کهگیلویه و بویراحمد" : name;
}
writeFileSync(output, JSON.stringify(geo));
console.log(`✅ ${geo.features.length} استان: ${statSync(input).size} → ${statSync(output).size} bytes`);
