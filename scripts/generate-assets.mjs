#!/usr/bin/env node
/**
 * تولید خودکار آیکون‌ها و اسکرین‌شات‌های PWA از روی لوگوی SVG.
 *
 * چرا لازم است؟ فایل‌های PNG باینری گاهی هنگام انتقال به مخزن/سرور جا می‌مانند
 * و در نتیجه PWABuilder خطای «Fix the links to your icons» می‌دهد.
 * این اسکریپت پیش از هر build اجرا می‌شود و همه تصاویر را روی سرور می‌سازد،
 * بنابراین همیشه موجود و سالم هستند.
 */
...
const tile = (x, y, w, icon, num, label, color) =>
  `<g>${card(x, y, w, 96)}<text x="${x + w / 2}" y="${y + 34}" font-size="22" text-anchor="middle" font-family="${F}">${icon}</text>` +
  `<text x="${x + w / 2}" y="${y + 64}" font-size="24" font-weight="bold" fill="${color}" text-anchor="middle" font-family="${F}">${num}</text>` +
  `<text x="${x + w / 2}" y="${y + 84}" font-size="13" fill="#64748b" text-anchor="middle" font-family="${F}">${label}</text></g>`;
const bars = (x, y, w, h, vals, color) => {
  const bw = w / vals.length - 10;
  return vals
    .map((v, i) => {
      const bh = Math.max(6, (v / Math.max(...vals)) * h);
      return `<rect x="${x + i * (bw + 10)}" y="${y + h - bh}" width="${bw}" height="${bh}" rx="5" fill="${color}" opacity="${0.55 + (i / vals.length) * 0.45}"/>`;
    })
    .join("");
};
const header = (w, title) =>
  `<rect width="${w}" height="72" fill="#0f766e"/>` +
  `<text x="${w - 24}" y="34" font-size="19" font-weight="bold" fill="#ffffff" text-anchor="end" font-family="${F}">ثبت اطلاعات کل</text>` +
...
${header(1280, "داشبورد مدیریت — گزارش و نمودار عملکرد")}
${tile(40, 100, 190, "\u{1F3E5}", "۱۲۸", "داروخانه‌ها", "#0f766e")}
${tile(248, 100, 190, "\u{1FA7A}", "۹۴", "پزشکان", "#0369a1")}
${tile(456, 100, 190, "\u{1F9FE}", "۳۱۲", "سفارشات", "#4f46e5")}
${tile(664, 100, 190, "\u{1F4CD}", "۵", "ویزیت فعال", "#059669")}
${tile(872, 100, 190, "\u{1F4DD}", "۳", "مرخصی", "#b45309")}
${tile(1080, 100, 160, "\u{1F464}", "۱۱", "کاربران", "#334155")}
${card(40, 220, 760, 300)}
<text x="770" y="252" font-size="16" font-weight="bold" fill="#0f172a" text-anchor="end" font-family="${F}">مقایسه فروش سال به سال</text>
${bars(80, 280, 660, 200, [40, 62, 55, 88, 72, 96, 110, 85, 120, 100, 135, 148], "#0d9488")}
${card(820, 220, 420, 300)}
<text x="1210" y="252" font-size="16" font-weight="bold" fill="#0f172a" text-anchor="end" font-family="${F}">سهم محصولات</text>
<g transform="translate(1030,390)">
  ...
  <text y="8" font-size="22" font-weight="bold" fill="#0f766e" text-anchor="middle" font-family="${F}">۳۱۲</text>
</g>
${card(40, 540, 1200, 150)}
<text x="1210" y="572" font-size="16" font-weight="bold" fill="#0f172a" text-anchor="end" font-family="${F}">فعالیت لحظه‌ای نمایندگان</text>
${[0, 1, 2]
  .map(
    (i) =>
      `<rect x="60" y="${590 + i * 32}" width="1160" height="26" rx="8" fill="#f8fafc"/>` +
      `<text x="1200" y="${608 + i * 32}" font-size="13" fill="#334155" text-anchor="end" font-family="${F}">${["علی رضایی — ثبت سفارش داروخانه هلال", "مریم احمدی — شروع ویزیت", "حسن کریمی — ثبت پزشک متخصص قلب"][i]}</text>`,
  )
  .join("")}
</svg>`;
...
${header(720, "پنل نماینده علمی")}
${card(24, 92, 672, 96)}
<text x="672" y="130" font-size="20" font-weight="bold" fill="#0f172a" text-anchor="end" font-family="${F}">سلام نماینده علمی</text>
<text x="672" y="162" font-size="14" fill="#64748b" text-anchor="end" font-family="${F}">امروز ۱۴۰۵/۰۶/۱۲ — عملکرد امروز شما</text>
<rect x="24" y="208" width="672" height="92" rx="18" fill="#0f766e"/>
<text x="668" y="248" font-size="19" font-weight="bold" fill="#ffffff" text-anchor="end" font-family="${F}">ثبت اطلاعات داروخانه</text>
<text x="668" y="276" font-size="13" fill="#c7f0ea" text-anchor="end" font-family="${F}">۱۲۸ رکورد ثبت‌شده</text>
<rect x="24" y="312" width="672" height="92" rx="18" fill="#0369a1"/>
<text x="668" y="352" font-size="19" font-weight="bold" fill="#ffffff" text-anchor="end" font-family="${F}">ثبت اطلاعات پزشک</text>
<text x="668" y="380" font-size="13" fill="#cfe9fb" text-anchor="end" font-family="${F}">۹۴ رکورد ثبت‌شده</text>
<rect x="24" y="416" width="672" height="92" rx="18" fill="#4f46e5"/>
<text x="668" y="456" font-size="19" font-weight="bold" fill="#ffffff" text-anchor="end" font-family="${F}">ثبت سفارشات داروخانه</text>
<text x="668" y="484" font-size="13" fill="#d8d6fb" text-anchor="end" font-family="${F}">۳۱۲ سفارش ارسال‌شده</text>
${["تردد", "منزل", "مرخصی", "اعلان", "افزودن", "گزارش"]
  .map((t, i) => {
    const x = 24 + (i % 3) * 228;
    const y = 528 + Math.floor(i / 3) * 116;
    return `${card(x, y, 208, 100)}<text x="${x + 104}" y="${y + 60}" font-size="16" font-weight="bold" fill="#334155" text-anchor="middle" font-family="${F}">${t}</text>`;
  })
  .join("")}
${card(24, 772, 672, 300)}
<text x="668" y="806" font-size="16" font-weight="bold" fill="#0f172a" text-anchor="end" font-family="${F}">روند فروش ماه به ماه</text>
${bars(56, 830, 610, 210, [30, 48, 42, 70, 62, 88, 96, 78, 110, 96, 124, 140], "#0ea5e9")}
${card(24, 1092, 672, 92)}
<text x="668" y="1128" font-size="15" font-weight="bold" fill="#0f766e" text-anchor="end" font-family="${F}">مسیریابی با نشان، بلد، ویز و گوگل‌مپ</text>
<text x="668" y="1156" font-size="13" fill="#64748b" text-anchor="end" font-family="${F}">کار آفلاین + ثبت لوکیشن دقیق GPS</text>
<rect x="0" y="1208" width="720" height="72" fill="#0f766e"/>
<text x="360" y="1252" font-size="15" font-weight="bold" fill="#ffffff" text-anchor="middle" font-family="${F}">ثبت اطلاعات کل — نسخه اندروید، ویندوز و وب</text>
</svg>`;
...
const BRAND = `<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720">
  ...
  <text x="640" y="560" font-size="46" font-weight="bold" fill="#ffffff" text-anchor="middle" font-family="${F}">ثبت اطلاعات کل</text>
  <text x="640" y="612" font-size="22" fill="#a7f3d0" text-anchor="middle" font-family="${F}">سامانه نمایندگان علمی — داروخانه، پزشک، سفارشات و تردد</text>
</svg>`;
...
/** آیکون maskable با حاشیه امن و پس‌زمینه روشن */
...
async function main() {
  ...
  try {
    ...
    console.warn("⚠️  sharp در دسترس نیست؛ آیکون‌ها هنگام اجرا به‌صورت داینامیک ساخته می‌شوند.");
    return;
  }
  ...
  // منابع SVG (متنی) کنار PNG ذخیره می‌شوند تا در صورت نبود PNG،
  // مسیرهای داینامیک بتوانند همان تصویر را در لحظه بسازند.
  ...
  console.log(
    `✅ ساخت تصاویر PWA کامل شد: ${ICON_SIZES.length} آیکون + ۲ maskable + ۳ اسکرین‌شات (${existsSync(ICONS) ? "ok" : "err"})`,
  );
}

if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch((e) => {
    console.error("خطا در ساخت تصاویر:", e?.message ?? e);
    process.exit(0); // build را متوقف نکن؛ مسیرهای داینامیک جایگزین هستند
  });
}
